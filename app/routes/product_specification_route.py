...
def get_product_specification(product_specification_id: int, db: db_dependency):
    return product_specification_controller.get_product_specification(product_specification_id, db)

@router.get('/', summary='GET ALL Specifications by Product', response_model=List[ProductSpecificationResponse])
def get_product_specifications(db: db_dependency, skip: int = 0, limit: int = 100):
    return product_specification_controller.get_product_specifications(db, skip, limit)
